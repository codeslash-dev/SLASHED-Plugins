...
= 0.6.12 =
* Fixed: Compare variables per-segment so family names keep natural order
* Fixed: Order variable dropdowns by scale instead of alphabetically
* Fixed: Use opposite-mode formula for alt selection swatch; widen tests
* Fixed: Resolve swatches for all remaining picker color tokens

= 0.6.11 =
* Fixed: Clean truncated class-hint tooltips; expand test coverage
* Fixed: Restore color-state swatches and class-dropdown hints

= 0.6.10 =
* Fixed: Replace stale .sf-aspect reference in bundle description text
...
* Fixed: Accept gradient values in token override validation

= 0.6.6 =
* Maintenance release.

= 0.6.5 =
* Fixed: Bump js-yaml and fast-uri to patched versions (audit high)
* Fixed: Ship the in-editor token panel assets so it actually loads

= 0.6.4 =
* Maintenance release.

= 0.6.3 =
* Fixed: Trim the SF color-picker class cheatsheet to genuinely colour/surface classes — drop `sf-bg-layer` (a layout primitive), `sf-color-pulse` (motion), `sf-text-protect` (legibility overlay) and `sf-surface-bg` (background-image preset)

= 0.6.2 =
* Added: Colour/surface class cheatsheet in the SF color picker (Bricks): a "Classes" view with category tabs that copies utility class names (`.sf-surface--*`, coloured `.sf-btn--*`, `.sf-marker--*`, text/link helpers) to the clipboard

= 0.6.1 =
* Added: Expose all semantic color tokens in SF color picker
