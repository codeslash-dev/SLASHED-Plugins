    ...
    <div class="space-y-1">
      {#each TOKEN_DOMAINS as d (d.id)}
        ...
        >
          ...
          {#if count > 0}
            <span class="text-[9px] font-bold text-indigo-400 bg-indigo-500/10 px-1.5 py-0.5 rounded-full font-mono shrink-0">{count}</span>
          {/if}
    ...
    <div class="space-y-1">
      {#each TOOLS as t (t.id)}
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <div class="text-[11px] font-semibold text-slate-300 group-hover:text-white transition-colors">{t.label}</div>
            <div class="text-[9px] text-slate-600">{t.desc}</div>
          </div>
          <span class="text-slate-700 group-hover:text-slate-500 text-[10px]">→</span>
        </button>
      {/each}
  ...
  <div>
    ...
    <div class="flex items-center gap-2 mb-2">
      ...
      <button
        ...
        class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-bold bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-600/30 transition-all cursor-pointer shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
      ><Save class="w-3 h-3" /></button>
    </div>

    {#if savedThemes.length === 0}
      <p class="text-[10px] text-slate-600">No saved themes yet. Configure tokens, then save.</p>
    {:else}
      <div class="grid grid-cols-2 gap-2">
        {#each savedThemes.slice(0, 6) as theme (theme.id)}
          ...
          >
            ...
            <div class="min-w-0">
              <div class="text-[11px] font-semibold text-slate-200 group-hover:text-white transition-colors truncate">{theme.name}</div>
