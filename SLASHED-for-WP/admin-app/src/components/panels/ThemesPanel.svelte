...
<div class="p-4 space-y-5 overflow-y-auto flex-1 h-full">
  ...
  <div class="rounded-xl border border-black/8 dark:border-white/8 bg-black/4 dark:bg-white/4 p-3 space-y-2">
    <div class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest">Save current</div>
    <div class="flex items-center gap-2">
      ...
      <button
        ...
        class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold bg-indigo-600/20 border border-indigo-500/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-600/30 transition-all cursor-pointer shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
      >
        <Save class="w-3 h-3" />
        Save
      </button>
      ...
      {#each themes as theme (theme.id)}
        ...
        <div class={`group rounded-xl border transition-all overflow-hidden ${
          activeId === theme.id
            ...
            : "border-black/8 dark:border-white/8 bg-black/4 dark:bg-white/4 hover:bg-black/6 dark:hover:bg-white/6 hover:border-black/14 dark:hover:border-white/14"
        }`}>
          <div class="flex items-start gap-3 p-4">
            <span class="text-2xl leading-none shrink-0 mt-0.5">{theme.icon}</span>
            <div class="flex-1 min-w-0">
              <div class="text-[12px] font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                {theme.name}
          ...
          {#if tokenCount > 0}
            <div class="px-4 pb-3 flex flex-wrap gap-1">
              {#each Object.entries(theme.overrides).slice(0, 5) as [k, v] (k)}
                <div class="flex items-center gap-1 bg-black/4 dark:bg-white/4 rounded px-1.5 py-0.5">
                  {#if k.includes("color") || k.includes("source")}
                    <div class="w-2.5 h-2.5 rounded-full border border-black/10 dark:border-white/10 shrink-0" style:background={v}></div>
                  {/if}
                  <span class="text-[8px] font-mono text-slate-400 dark:text-slate-600">{k.replace("--sf-", "")}</span>
                </div>
              {/each}
              {#if tokenCount > 5}
                <span class="text-[8px] text-slate-400 dark:text-slate-600 px-1.5 py-0.5">+{tokenCount - 5} more</span>
              {/if}
            </div>
          {/if}
        </div>
  ...
  {#if overridesCount > 0}
    <button
      onclick={onResetAll}
      class="w-full py-2 rounded-lg text-[11px] font-bold border border-rose-500/30 text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer"
